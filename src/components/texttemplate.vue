  ...
  methods: {
    ...
    selectionChange: function (selection) {
      ...
      for (let i = 0; i < selection.length; i++) {
        selectionId.push(selection[i].actId)
      }
      this.$store.commit('activities/reserveChecked', selectionId)
    },
    ...
    handleChangeSome: function (changeCondition) {
      let ids = this.$store.state.activities.tableChecked.length
      if (ids === 0) {
        return
      }
      this.$confirm('是否将已选中的' + ids + '条志愿服务活动的状态批量修改为【' + changeCondition + '】?', '操作提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
        // 发出服务器通过申请
        this.$store.commit('activities/changeSomeActCondition', changeCondition)
        this.$message({
          type: 'success',
          message: '已将活动状态修改为【' + changeCondition + '】！'
        })
      }).catch(() => {
        this.$message({
          type: 'info',
    ...
    handleChangeOne: function (row, changeCondition) {
      if (row.actCondition === changeCondition) {
        return
      }
      let change = {}
      change.actId = row.actId
      change.changeCondition = changeCondition
      // 通过本行
      this.$confirm('是否将编号为【' + change.actId + '】的活动状态设置为【' + changeCondition + '】?', '操作提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
        // 发出服务器通过申请
        this.$store.commit('activities/changeOneActCondition', change)
        this.$message({
          type: 'success',
          message: '已将编号为【' + change.actId + '】活动状态修改为【' + changeCondition + '】！'
        })
      }).catch(() => {
        this.$message({
          type: 'info',
